...
export default function PiePinsCost() {
  const {
    state: { pins },
  } = useValue();
  ...
  useEffect(() => {
    let free = 0,
      ...
      moreThan35 = 0;
    pins.forEach((pin) => {
      if (pin.price === 0) return free++;
      if (pin.price < 15) return lessThan15++;
      if (pin.price <= 35) return between15And35++;
      moreThan35++;
    });
    setCostGroups([
  ...
  return (
    ...
    >
      ...
      <Stack gap={2}>
        <Typography variant="h6">pins Cost</Typography>
        <Box sx={{ display: 'flex', gap: 3, flexWrap: 'wrap' }}>
          {COLORS.map((color, i) => ( //radum mau
